...
func GetSurveyQuestions(database *sql.DB, surveyID int) ([]survey.Question, error) {
	...
	// Get Questions
	var questions = []survey.Question{survey.Question{ID: result[0].questionID,
	...
	for _, row := range result {
		if row.questionID != currentQuestionID {
			...
			questions = append(questions, survey.Question{ID: row.questionID,
		...
		for j := 0; j < len(result); j++ {
			if questions[i].ID == result[j].questionID {
				questions[i].Answers = append(questions[i].Answers, 
					survey.Answer{ID: result[j].answerID,
